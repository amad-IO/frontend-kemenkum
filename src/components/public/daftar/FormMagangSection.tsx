...
const magangSchema = z.object({
  ...
  // Kelompok anggota (opsional, max 2)
...
const FormMagangSection = ({ onSuccess }: Props) => {
  ...
  const [periode, setPeriode] = useState(getPeriode('magang'))
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            placeholder="Input Nama Sekolah / Universitas"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          <label className="text-sm font-semibold text-neutral-text">
            Posisi Yang Dipilih <span className="text-red-500">*</span>
          </label>
          <select {...register('position_id')} className="input-field" disabled={loadingPosisi}>
            <option value="">
              {loadingPosisi ? 'Memuat posisi...' : 'Pilih Posisi Yang Diinginkan'}
            </option>
            {posisiList.map((p) => (
          ...
          {errors.position_id && (
            <p className="text-xs text-red-500">{errors.position_id.message}</p>
          )}
        </div>
      ...
      {jenisPeserta === 'kelompok' && (
        <div className="flex flex-col gap-3">
          ...
          {fields.length < 2 && (
            ...
            >
              <Plus size={16} />
              Tambah Anggota {fields.length + 1}
            </button>
          )}
        </div>
      )}
