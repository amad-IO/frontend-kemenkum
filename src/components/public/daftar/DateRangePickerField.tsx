...
const DateRangePickerField = ({
  ...
  const chooseDate = (date: Date) => {
    ...
    if (step === 'start') {
      setTempStart(value)
      if (tempEnd && tempEnd < value) {
        setTempEnd('')
      }
      return
    }

    if (tempStart && value < tempStart) return
    setTempEnd(value)
  }
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-neutral-text/40 px-3 py-4 sm:items-center">
            ...
            <div className="grid md:grid-cols-[1.12fr_0.88fr]">
              <div className="p-5">
                ...
                <div className="grid grid-cols-7 gap-y-2">
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                    <span key={day} className="py-1 text-center text-xs font-bold text-neutral-muted">
                      {day}
                    </span>
                  ))}
              ...
              <div className="border-t border-neutral-border p-5 md:border-l md:border-t-0">
                <div className="flex flex-col gap-5">
                  <div>
                    <label className="mb-2 block text-sm font-bold text-neutral-text">
                      Start date*
                    </label>
                    <button
                      type="button"
